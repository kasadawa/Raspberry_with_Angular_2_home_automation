...
//loaing the index.html page located in /views folder
...
router.put('/sw-device/:id',(req,res,next)=>{
    ...
    if(!cur_device){
        ...
        db.devices.update({_id: mongojs.ObjectId(req.params.id)},{$set: updated_device},{},(err,device)=>{
            if(err){
                res.send(err);
            }else{
